...
TEST(ColorTest, ColorAddition)
{
    ...
    // 加算演算子のテスト
    result = c1 + c2;
    EXPECT_DOUBLE_EQ(result.r, 0.5 + 0.1) << "r of c1 + c2 should be 0.5 + 0.1";
    EXPECT_DOUBLE_EQ(result.g, 0.5 + 0.2) << "g of c1 + c2 should be 0.5 + 0.2";
    EXPECT_DOUBLE_EQ(result.b, 0.5 + 0.3) << "b of c1 + c2 should be 0.5 + 0.3";

    // 加算代入演算子のテスト
    result += c2;
    EXPECT_DOUBLE_EQ(result.r, 0.5 + 0.1 + 0.1) << "r of result += c2 should be 0.5 + 0.1 + 0.1";
    EXPECT_DOUBLE_EQ(result.g, 0.5 + 0.2 + 0.2) << "g of result += c2 should be 0.5 + 0.2 + 0.2";
    EXPECT_DOUBLE_EQ(result.b, 0.5 + 0.3 + 0.3) << "b of result += c2 should be 0.5 + 0.3 + 0.3";
}
...
TEST(ColorTest, ColorScalarAddition)
{
    ...
    result = c + 0.1;
    EXPECT_DOUBLE_EQ(result.r, 0.1 + 0.1);
    EXPECT_DOUBLE_EQ(result.g, 0.2 + 0.1);
    EXPECT_DOUBLE_EQ(result.b, 0.3 + 0.1);

    result = 0.1 + c;
    EXPECT_DOUBLE_EQ(result.r, 0.1 + 0.1);
    EXPECT_DOUBLE_EQ(result.g, 0.1 + 0.2);
    EXPECT_DOUBLE_EQ(result.b, 0.1 + 0.3);

    result += 0.1;
    EXPECT_DOUBLE_EQ(result.r, 0.1 + 0.1 + 0.1);
    EXPECT_DOUBLE_EQ(result.g, 0.1 + 0.2 + 0.1);
    EXPECT_DOUBLE_EQ(result.b, 0.1 + 0.3 + 0.1);
}
...
TEST(ColorTest, ColorScalarMultiplication)
{
    ...
    result *= 5;
    EXPECT_DOUBLE_EQ(result.r, 5 * 0.1 * 5);
    EXPECT_DOUBLE_EQ(result.g, 5 * 0.2 * 5);
    EXPECT_DOUBLE_EQ(result.b, 5 * 0.3 * 5);
}
...
// ostream <<
TEST(ColorTest, OstreamOutput) {
    Color c(1.0, 0.5, 0.0);
    std::ostringstream oss;
    oss << c;
    EXPECT_EQ(oss.str(), "(1, 0.5, 0)");
}
