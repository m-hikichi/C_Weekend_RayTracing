...
int main()
{
    ...
    const Vec3 look_from = Vec3(3, 3, 2);
    const Vec3 look_at = Vec3(0, 0, -1);
    const Ray view_direction = Ray(look_from, look_at - look_from);
    const double focus_distance = (look_at - look_from).norm();
    const double vertical_fov = M_PI / 9;
    ThinLensCamera camera(image_width, image_height, view_direction, 2.0, focus_distance, vertical_fov);
    ...
    camera.get_image().save_png("../image/05-03_camera_contrast.png");
}
